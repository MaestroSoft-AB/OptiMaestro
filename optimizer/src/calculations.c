...
static inline int
calc_results_create(Calc_Results* _Res, const Facility_Config* _F, const Electricity_Spots* _S,
...
static inline int calc_fetch_input_data(Electricity_Spots* _S, Weather* _W, SqlHelper* _Sql,
...
void calc_daily_averages_threadtask(void* _context) {
  ...
  if (!T_Args || !F_Ptr || !C_Args || !C_Args->calcs_dir || !C_Args->data_dir || !Sql) {
  ...
  int res = calc_fetch_input_data(S_Ptr, W_Ptr, Sql, F_Ptr, C_Args->data_dir);
  if (res != SUCCESS) {
  ...
  char* filename_json =
      calc_name_get_daily(C_Args->calcs_dir, F_Ptr->name, "json", epd, time(NULL));
  if (!filename_json) {
...
calc_results_create(Calc_Results* _Res, const Facility_Config* _F, const Electricity_Spots* _S,
  ...
  if (!_S || _interval == 0                     // don't divide by zero
      ...
      || (_W && _W->values[0].timestamp !=
                    _S->prices[0].time_start) // both E+W first index must have same start time
...
static inline int calc_summary_create(const Calc_Results* _Res, const Facility_Config* _F,
  ...
  /* Table Header */
  if (_Res->solar_gains && _Res->solar_gains_deviation) {
    fprintf(file, "%-25s | %-7s | %-7s | %-9s | %-10s | %-11s\n", "Timestamp", "Spot", "Dev(%)",
            "Cheapness", "SolarGains", "SolarDev(%)");
    fprintf(
        file,
        "------------------------------------------------------------------------------------\n");
  } else {
    fprintf(file, "%-25s | %-7s | %-7s | %-9s\n", "Timestamp", "Spot", "Dev(%)", "Cheapness");
    fprintf(file, "---------------------------------------------------------\n");
  }
  /* Table Row data */
  for (unsigned int i = 0; i < _Res->count; i++) {
    const char* ts_str    = parse_epoch_to_iso_full_datetime_string(&_Res->timestamps[i], 1);
    ...
    if (_Res->solar_gains && _Res->solar_gains_deviation) {
      fprintf(file, "%-19s | %7.2f | %7.2f | %-9s | %10.2f | %11.2f\n", ts_str,
              _Res->spot_prices[i], _Res->spot_prices_deviation[i], cheap_str, _Res->solar_gains[i],
              _Res->solar_gains_deviation[i]);
    } else {
      fprintf(file, "%-19s | %7.2f | %7.2f | %-9s\n", ts_str, _Res->spot_prices[i],
...
static inline int calc_fetch_input_data(Electricity_Spots* _S,
                                        Weather*           _W, // optional
                                        SqlHelper* _Sql, const Facility_Config* _F,
  ...
  res = ech_get_spots_range(_Sql, _S, _data_dir, _F->price_class, _F->currency, start, end);
  ...
  if (_W) {
    ...
    res = wch_get_weather_range(_Sql, _W, _F->lat, _F->lon, _F->panel->tilt, _F->panel->azimuth,
                                true, start, end);
    if (res != SUCCESS) {
